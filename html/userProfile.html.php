...
<body class="profileBody">
    <div class="homeBar">
        <h1>Smith Friends</h1>
        <div class="nav-links">
            <button class="edit-profile-btn" onclick="openUserEditPage()">Edit Profile</button>
    ...
    function getAllListings() {
            const events = <?php echo json_encode(getUserEvents("../data/listing.json", $_SESSION['username'])); ?>; 
            
            const container = document.getElementById('listingsContainer');

            if (events.length == 0) {
                container.innerHTML = "<h5> Create Events to See Your Events :) </h5>"; 
            }
            else {
